...
class ValidationServiceAdditionalSpec extends FunSuite:
  ...
  test("validateJob - duplicate step IDs in composite step fails") {
    ...
    ValidationService.validateJob(job) match
      ...
      case cats.data.Validated.Valid(_) => fail("Expected validation error for duplicate step IDs in composite step")
  }
  ...
  test("validatePipeline - invalid label fails") {
    ...
    ValidationService.validatePipeline(pipeline) match
      case Left(e) => 
        assert(e.errors.exists(_.message.contains("must contain only alphanumeric characters, underscores, and hyphens")))
  ...
  test("validateJob - invalid label fails") {
    ...
    ValidationService.validateJob(job) match
      case cats.data.Validated.Invalid(e) => 
        assert(e.toList.exists(_.message.contains("must contain only alphanumeric characters, underscores, and hyphens")))
