...
class ValidationServiceSpec extends FunSuite:
  ...
  test("validateJob - self-dependency fails") {
    ...
    ValidationService.validateJob(job) match
      case cats.data.Validated.Invalid(e) => assert(e.toList.exists(_.message.contains("cannot depend on itself")))
  ...
  test("validateJob - zero CPU fails") {
    ...
    ValidationService.validateJob(job) match
      case cats.data.Validated.Invalid(e) => assert(e.toList.exists(_.message.contains("CPU must be positive")))
  ...
  test("validateJob - negative memory fails") {
    ...
    ValidationService.validateJob(job) match
      case cats.data.Validated.Invalid(e) => assert(e.toList.exists(_.message.contains("Memory must be positive")))
  ...
  test("validateJob - negative GPU fails") {
    ...
    ValidationService.validateJob(job) match
      case cats.data.Validated.Invalid(e) => assert(e.toList.exists(_.message.contains("GPU count cannot be negative")))
  ...
  test("validateJobs - missing dependency fails") {
    ...
    ValidationService.validateWorkflow(workflow) match
      case cats.data.Validated.Invalid(e) => assert(e.toList.exists(_.message.contains("Unknown dependencies")))
  ...
  test("validateJob - empty matrix key fails") {
    ...
    ValidationService.validateJob(job) match
      case cats.data.Validated.Invalid(e) => assert(e.toList.exists(_.message.contains("Matrix key cannot be empty")))
  ...
  test("validateJob - too many matrix combinations fails") {
    ...
    ValidationService.validateJob(job) match
      case cats.data.Validated.Invalid(e) => assert(e.toList.exists(_.message.contains("too many combinations")))
  ...
  test("validateJob - empty container image fails") {
    ...
    ValidationService.validateJob(job) match
      case cats.data.Validated.Invalid(e) => assert(e.toList.exists(_.message.contains("Container image cannot be empty")))
  ...
  test("validateStep - negative retry delay fails") {
    ...
    ValidationService.validateStep(step) match
      case cats.data.Validated.Invalid(e) => assert(e.toList.exists(_.message.contains("delay must be positive")))
  ...
  test("validatePipeline - complex valid pipeline passes") {
    ...
    val job1 = Job.one(JobId("build"), step1)
      .~>(step2)
      .~>(step3)
    
    val job2 = Job.one(JobId("deploy"), Step.Run(Command.Exec("deploy", List("prod")))(using StepMeta()))
      .needs(JobId("build"))
