...
/** Validator for JWT tokens
  *
  * Note: This is a basic implementation for structural validation. For production use, integrate with a
  * proper JWT library like:
  *   - jwt-scala
  *   - jose4j
  *   - nimbus-jose-jwt
  */
class JwtValidator(config: JwtConfig):
  ...
  /** Validate a JWT token string
    ...
    * @return
    *   Either a validation error or the validated claims
    */
  ...
  private def parseToken(token: String): Either[ValidationError, TokenParts] =
    ...
    else Right(TokenParts(parts(0), parts(1), parts(2)))
  ...
  private def decodeBase64(encoded: String): Either[ValidationError, String] =
    ...
    }.toEither.left.map(e => ValidationError(s"Failed to decode base64: ${e.getMessage}", Some("jwt.decode")))
  ...
  private def validateClaims(claims: JwtClaims): Either[ValidationError, Unit] =
    ...
    claims.expiresAt match
      case Some(exp) if exp + config.clockSkewSeconds < now =>
        return Left(
          ValidationError(s"JWT token has expired (exp=$exp, now=$now)", Some("jwt.validation.expired"))
        )
      case _ => ()
    ...
    claims.notBefore match
      case Some(nbf) if nbf - config.clockSkewSeconds > now =>
        return Left(
          ValidationError(s"JWT token not yet valid (nbf=$nbf, now=$now)", Some("jwt.validation.nbf"))
        )
      case _ => ()
    ...
    if config.expectedAudience.nonEmpty then
      claims.audience match
        case Some(aud) if !aud.exists(config.expectedAudience.contains) =>
          return Left(
            ValidationError(
              s"JWT token audience mismatch (expected=${config.expectedAudience.mkString(",")}, got=${aud
                  .mkString(",")})",
