...
/**
 * Represents a single persisted job execution row.
 * The row tracks scheduling, execution windows, resource requests/usage,
 * retry state and terminal results for orchestration and auditing.
 *
 * @param jobExecutionId        Primary key for this job execution attempt
 * @param workflowExecutionId   Owning workflow execution identifier
 * @param executionId           Denormalized pipeline execution identifier
 * @param jobId                 Logical job identifier from the pipeline spec
 * @param jobName               Human-friendly job name
 * @param status                Current execution status (pending, running, etc.)
 * @param dependencies          JSON encoded upstream dependency graph
 * @param readyAt               When all dependencies were satisfied
 * @param scheduledAt           When the job was scheduled onto an executor
 * @param executorId            UUID of the executor assigned to this job
 * @param assignedAt            Timestamp for assignment to an executor
 * @param startedAt             When the executor actually began running the job
 * @param completedAt           When execution finished (success or failure)
 * @param timeoutAt             Deadline after which the job should be aborted
 * @param requestedCpuMilli     Requested CPU in milli-cores
 * @param requestedMemoryMib    Requested memory in MiB
 * @param requestedGpu          Requested GPU device count
 * @param requestedDiskMib      Requested disk space in MiB
 * @param actualCpuMilliSeconds Actual CPU consumption in milli-seconds
 * @param actualMemoryMibSeconds Actual memory consumption in MiB-seconds
 * @param attemptNumber         1-based attempt counter for retries
 * @param maxAttempts           Maximum allowed attempts
 * @param exitCode              Exit code emitted by the job process
 * @param errorMessage          Error message when the job fails
 * @param matrixCoordinates     JSON coordinates for matrix-expanded jobs
 */
